#!/usr/bin/env python3
"""Compatibility CLI for ARC-5020 RAID3 image reconstruction."""
...
from areca import ArecaArray, ArecaError, RaidLevel, parse_size, xor_blocks


def validate_members(paths: list[str]) -> ArecaArray:
    array = ArecaArray.assemble(paths)
    if array.level != RaidLevel.RAID3:
        raise ArecaError(f"expected RAID3, detected {array.level.value}")
    return array
