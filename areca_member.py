...
def inspect(path: str) -> Inspection:
    ...
    try:
        ...
        for lba in range(VOLUME_RECORD_FIRST_LBA, VOLUME_RECORD_LAST_LBA + 1):
            record = pread_exact(fd, SECTOR_SIZE, lba * SECTOR_SIZE)
            if record[:8] == VOLUME_MAGIC:
                records.append((lba, record))
        ...
        volumes: list[Volume] = []
        for record_lba, record in records:
            sectors = struct.unpack_from("<I", record, 8)[0]
            duplicate_sectors = struct.unpack_from("<I", record, 20)[0]
            if sectors == 0:
                raise ArecaError(f"Volume record at LBA {record_lba} has zero length")
            if duplicate_sectors != sectors:
                warnings.append(
                    f"Volume LBA {record_lba}: sector-count copy at +0x14 "
            ...
            if stripe_sectors == 0 or stripe_sectors != stripe_sectors_copy:
                warnings.append(
                    f"Volume LBA {record_lba}: stripe fields at +0x28/+0x2a "
                    f"are {stripe_sectors}/{stripe_sectors_copy}"
                )
            if raid_level_code != raid_level_code_copy:
                warnings.append(
                    f"Volume LBA {record_lba}: RAID-level fields at +0x2c/+0x2d "
        ...
        if len(volumes) > 1:
            warnings.append(
                "Multiple Volume records exist. Mapping is disabled because this "
                "prototype only validates a single-volume RAID1 member."
            )
        ...
        return Inspection(
            ...
            volume_magic_lbas=[lba for lba, _ in records],
...
def print_human(result: Inspection) -> None:
    print(f"Areca metadata: detected")
    ...
    for index, volume in enumerate(result.volumes):
        ...
        print(f"  Metadata record LBA: {volume.record_lba}")
        print(
